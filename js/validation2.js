...
(function () {
  ...
  function validateRequired(el) {
    if (isRequired(el)) {                           // Is this element required?
      ...
      if (!valid) {                                 // If valid variable holds false
        setErrorMessage(el,  'Please enter required elements in field');  // Set the error message
      }
  ...
  // Check if the bio is less than or equal to 200 characters
  function validateBio() {
    var bio = document.getElementById('bio');
    var valid = bio.value.length <= 200;
    if (!valid) {
      setErrorMessage(bio, 'Please make sure your bio does not exceed 200 characters');
    }
    return valid;
  }
  ...
  function validatePassword() {
    ...
    if (!valid) {
      setErrorMessage(password, 'Please make sure your password has at least 10 characters');
    }
    return valid;
  }
